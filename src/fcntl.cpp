...
result<file_descriptor> open(const char *path, const capi::access_mode mode, const capi::open_flag flags) noexcept {
  PPOSIX_COMMON_RESULT_DOUBLE_WRAP_IMPL(file_descriptor, raw_fd, ::open, path, underlying_v(mode) | underlying_v(flags))
}

result<file_descriptor> open(const char *path, capi::access_mode mode, capi::open_flag flags, capi::permission permission) noexcept {
  PPOSIX_COMMON_RESULT_DOUBLE_WRAP_IMPL(file_descriptor, raw_fd, ::open, path, underlying_v(mode) | underlying_v(flags), underlying_v(permission))
}
