...
// Set socket option
std::error_code setsockopt(const socket_fd &fd, socket_level l, socket_option o,
                           any_cspan val) noexcept {
  const auto error =
...
std::error_code setsockopt_int(const socket_fd &fd, socket_level l, socket_option o,
                               int i) noexcept {
  return pposix::setsockopt(fd, l, o, any_cspan{i});
}

std::error_code setsockopt_bool(const socket_fd &fd, socket_level l, socket_option o,
                                bool b) noexcept {
...
std::error_code setsockopt(const socket_fd &fd, socket_level l, debug d) noexcept {
  return setsockopt_bool(fd, l, socket_option::debug, underlying_value(d));
}

std::error_code setsockopt(const socket_fd &fd, socket_level l, broadcast b) noexcept {
  return setsockopt_bool(fd, l, socket_option::broadcast, underlying_value(b));
}

std::error_code setsockopt(const socket_fd &fd, socket_level l, reuseaddr r) noexcept {
  return setsockopt_bool(fd, l, socket_option::reuseaddr, underlying_value(r));
}

std::error_code setsockopt(const socket_fd &fd, socket_level l, keepalive k) noexcept {
  return setsockopt_bool(fd, l, socket_option::keepalive, underlying_value(k));
}

std::error_code setsockopt(const socket_fd &fd, socket_level l, linger lin) noexcept {
  const ::linger &posix_linger = lin.get();
  return setsockopt(fd, l, socket_option::linger, any_cspan{posix_linger});
}

std::error_code setsockopt(const socket_fd &fd, socket_level l, oobinline o) noexcept {
  return setsockopt_bool(fd, l, socket_option::oobinline, underlying_value(o));
}

std::error_code setsockopt(const socket_fd &fd, socket_level l, sndbuf s) noexcept {
  return setsockopt_int(fd, l, socket_option::sndbuf, underlying_value(s));
}

std::error_code setsockopt(const socket_fd &fd, socket_level l, rcvbuf r) noexcept {
  return setsockopt_int(fd, l, socket_option::rcvbuf, underlying_value(r));
}

std::error_code setsockopt(const socket_fd &fd, socket_level l, dontroute d) noexcept {
  return setsockopt_bool(fd, l, socket_option::dontroute, underlying_value(d));
}

std::error_code setsockopt(const socket_fd &fd, socket_level l, rcvlowat r) noexcept {
  return setsockopt_int(fd, l, socket_option::rcvlowat, underlying_value(r));
}

std::error_code setsockopt(const socket_fd &fd, socket_level l, rcvtimeo r) noexcept {
  const ::timeval &val = r.get();
  return setsockopt(fd, l, socket_option::rcvtimeo, any_cspan{val});
}

std::error_code setsockopt(const socket_fd &fd, socket_level l, sndlowat s) noexcept {
  return setsockopt_int(fd, l, socket_option::sndlowat, underlying_value(s));
}

std::error_code setsockopt(const socket_fd &fd, socket_level l, sndtimeo r) noexcept {
  const ::timeval &val = r.get();
  return setsockopt(fd, l, socket_option::sndtimeo, any_cspan{val});
}

// Get socket option
result<socklen_t> getsockopt(const socket_fd &fd, socket_level l, socket_option o,
...
result<Result> getsockopt_int(const socket_fd &fd, socket_level l, socket_option o) noexcept {
  int val{};
...
template <class Result>
result<Result> getsockopt_bool(const socket_fd &fd, socket_level l, socket_option o) noexcept {
  int val{};
...
template <class Result>
result<Result> getsockopt_struct(const socket_fd &fd, socket_level l, socket_option o) noexcept {
...
template <>
result<debug> getsockopt<debug>(const socket_fd &fd, socket_level l) noexcept {
  return detail::getsockopt_bool<debug>(fd, l, socket_option::debug);
}

template <>
result<broadcast> getsockopt<broadcast>(const socket_fd &fd, socket_level l) noexcept {
  return detail::getsockopt_bool<broadcast>(fd, l, socket_option::broadcast);
}

template <>
result<reuseaddr> getsockopt<reuseaddr>(const socket_fd &fd, socket_level l) noexcept {
  return detail::getsockopt_bool<reuseaddr>(fd, l, socket_option::reuseaddr);
}

template <>
result<acceptconn> getsockopt<acceptconn>(const socket_fd &fd, socket_level l) noexcept {
  return detail::getsockopt_bool<acceptconn>(fd, l, socket_option::acceptconn);
}

template <>
result<keepalive> getsockopt<keepalive>(const socket_fd &fd, socket_level l) noexcept {
  return detail::getsockopt_bool<keepalive>(fd, l, socket_option::keepalive);
}

template <>
result<linger> getsockopt<linger>(const socket_fd &fd, socket_level l) noexcept {
  return detail::getsockopt_struct<linger>(fd, l, socket_option::linger);
}

template <>
result<oobinline> getsockopt<oobinline>(const socket_fd &fd, socket_level l) noexcept {
  return detail::getsockopt_bool<oobinline>(fd, l, socket_option::oobinline);
}

template <>
result<sndbuf> getsockopt<sndbuf>(const socket_fd &fd, socket_level l) noexcept {
  return detail::getsockopt_int<sndbuf>(fd, l, socket_option::sndbuf);
}

template <>
result<rcvbuf> getsockopt<rcvbuf>(const socket_fd &fd, socket_level l) noexcept {
  return detail::getsockopt_int<rcvbuf>(fd, l, socket_option::rcvbuf);
}

template <>
result<error> getsockopt<error>(const socket_fd &fd, socket_level l) noexcept {
  return detail::getsockopt_int<error>(fd, l, socket_option::error);
}

template <>
result<socket_type> getsockopt<socket_type>(const socket_fd &fd, socket_level l) noexcept {
  return detail::getsockopt_int<socket_type>(fd, l, socket_option::type);
}

template <>
result<dontroute> getsockopt<dontroute>(const socket_fd &fd, socket_level l) noexcept {
  return detail::getsockopt_bool<dontroute>(fd, l, socket_option::dontroute);
}

template <>
result<rcvlowat> getsockopt<rcvlowat>(const socket_fd &fd, socket_level l) noexcept {
  return detail::getsockopt_int<rcvlowat>(fd, l, socket_option::rcvlowat);
}

template <>
result<rcvtimeo> getsockopt<rcvtimeo>(const socket_fd &fd, socket_level l) noexcept {
  return detail::getsockopt_struct<rcvtimeo>(fd, l, socket_option::rcvtimeo);
}

template <>
result<sndlowat> getsockopt<sndlowat>(const socket_fd &fd, socket_level l) noexcept {
  return detail::getsockopt_int<sndlowat>(fd, l, socket_option::sndlowat);
}

template <>
result<sndtimeo> getsockopt<sndtimeo>(const socket_fd &fd, socket_level l) noexcept {
