...
result<ioctl_result> ioctl(const raw_fd &fd, ioctl_set_write_option write_option) noexcept {
  return pposix::ioctl(fd, ioctl_request{I_SWROPT}, underlying_value(write_option.write_option));
}

result<ioctl_write_option> ioctl(const raw_fd &fd, decltype(ioctl_get_write_option)) noexcept {
  int write_option{};

  const auto res{pposix::ioctl(fd, ioctl_request{I_GWROPT}, &write_option)};
...
result<ioctl_new_file_descriptor> ioctl(const raw_fd &fd,
  ...
  ::strrecvfd new_fd{};
  const auto res{pposix::ioctl(fd, ioctl_request{I_RECVFD}, &new_fd)};
...
result<ioctl_module_name_list> ioctl(const raw_fd &fd,
  ...
  const auto res{pposix::ioctl(fd, ioctl_request{I_LIST}, &module_name_list)};
